...
import { render } from "../../server/site-renderer/entry-server.js";
...
await writeFile("../server/site-renderer/template.html", template);
const routes = Object.keys(routeMeta);
for (const route of [...routes, "/404"]) {
  const html = template.replace("<!--seo-head-->", renderHead(getPageMeta(route))).replace("<!--app-html-->", await render(route)).replace("<!--media-data-->", '<script type="application/json" id="site-media">{}</script>');
