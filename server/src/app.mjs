...
export async function createApp(config) {
  ...
  app.use(helmet({ crossOriginResourcePolicy: { policy: "same-site" },
    contentSecurityPolicy: { directives: { "default-src": ["'self'"], "script-src": ["'self'"], "style-src": ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com"], "font-src": ["'self'", "https://fonts.gstatic.com"], "img-src": ["'self'", "https:", "data:", "blob:"], "connect-src": ["'self'"], "frame-src": ["https://www.google.com"], "object-src": ["'none'"], "frame-ancestors": ["'none'"], "upgrade-insecure-requests": config.production ? [] : null } },
  ...
  app.get("/api/media", (_req, res) => res.set("Cache-Control", "no-cache").json({ revision: store.revision(), images: store.manifest() }));
  app.post("/api/admin/login", async (req, res) => {
    if (!req.is("application/json")) throw error(415, "Send a JSON login request.");
    if (!store.attempt("login:" + req.ip, 10)) throw error(429, "Too many sign-in attempts. Try again in 15 minutes.");
    ...
    store.createSession(tokenHash(token), user.id, csrf, Date.now() + cookieOptions.maxAge);
    res.cookie("vc_admin", token, cookieOptions).json({ email: user.email, csrf });
  });
  app.use("/api/admin", (req, _res, next) => {
    const token = readSessionCookie(req);
    req.session = /^[A-Za-z0-9_-]{43}$/.test(token) ? store.session(tokenHash(token)) : null;
  ...
  app.post("/api/admin/logout", (req, res) => { store.deleteSession(req.session.hash); res.clearCookie("vc_admin", { ...cookieOptions, maxAge: undefined }).json({ ok: true }); });
  const detail = item => ({ ...item, ...store.image(item.id) });
  app.get("/api/admin/images", (_req, res) => res.json({ images: catalog.map(detail), revision: store.revision() }));
  ...
  const checkVersion = (req, _res, next) => {
    ...
    if (store.image(req.image.id).version !== req.imageVersion) return next(error(409, "This image has changed. Refresh before saving."));
    if (!store.attempt("edit:" + req.session.admin_id, 120)) return next(error(429, "Too many edits. Please try later."));
    next();
  };
  const altText = value => {
  ...
  app.patch("/api/admin/images/:id", checkVersion, (req, res) => {
    const alt = altText(req.body?.alt);
    const previous = store.image(req.image.id).value;
    store.saveImage(req.image.id, { ...previous, alt }, req.imageVersion, req.session.admin_id);
    pageCache.clear(); res.json({ image: detail(req.image) });
  });
  app.put("/api/admin/images/:id", checkVersion, (req, res, next) => {
    ...
    upload(req, res, async uploadError => {
      ...
      try {
        ...
        store.saveImage(req.image.id, { ...generated.image, alt }, req.imageVersion, req.session.admin_id);
        pageCache.clear(); res.json({ image: detail(req.image) });
      } catch (err) { if (generated) await generated.cleanup(); next(err); }
      finally { uploading = false; }
    });
  });
  app.post("/api/admin/images/:id/restore", checkVersion, (req, res) => {
    store.saveImage(req.image.id, null, req.imageVersion, req.session.admin_id);
    pageCache.clear(); res.json({ image: detail(req.image) });
  });
  ...
  app.get("/manifest.json", async (_req, res) => {
    ...
    const icon = store.manifest()["site-icon"];
  ...
  app.get(["/", "/{*path}"], async (req, res) => {
    ...
    if (!pageCache.has(key)) {
      ...
      if (revision === store.revision()) pageCache.set(key, html);
